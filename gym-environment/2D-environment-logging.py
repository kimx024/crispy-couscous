...
def main():
  ...
  for i in range(50):  # Let's run it for 2 episodes for simplicity
    ...
    logging.info(f"Action taken: Shot towards {direction}")
    logging.info(f"Final ball position: x={env.ball[0]}, y={env.ball[1]}")
    logging.info(f"Goalkeeper's final position: x={env.goalkeeper_position[0]}, y={env.goalkeeper_position[1]}")
    logging.info(f"Goalkeeper dove to: {info['goalkeeper_section']}, Reward: {reward}")
    logging.info("---" * 10)
  env.close()
