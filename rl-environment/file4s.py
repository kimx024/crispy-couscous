...
class Environment(gym.Env):
    ...
    def __init__(self, file_path):
        ...
        self.goalkeeper_start_pos = self.middle
        self.goalkeeper_target_pos = self.middle
        self.goalkeeper_progress = 0
        self.goalkeeper_animation = False
        self.goalkeeper_width = 60
        self.goalkeeper_height = 200
    ...
    def reset(self, *, seed: int = None, options: dict = None) -> tuple[np.ndarray, dict]:
        ...
        # Reset positioning
        self.ball_start_pos = np.array(self.middle)
        self.ball_target_pos = np.array(self.middle)
        self.goalkeeper_start_pos = self.middle
        self.goalkeeper_target_pos = self.middle
    ...
    def step(self, action):
        ...
        info_step = {"Episode: ": self.current_step,
    ...
    def animate_ball(self):
        if self.ball_animation:
            step_increment = 0.07
            self.ball_progress += step_increment

            predicted_position = self.predict_ball_position()
            current_goalkeeper_pos = self.goalkeeper_start_pos + (
                    self.goalkeeper_target_pos - self.goalkeeper_start_pos) * self.ball_progress

            if self.check_collision_in_frame(predicted_position, current_goalkeeper_pos):
                self.ball_animation = False
                self.current_reward = self.reward_lose
                self.collision_occurred = True
                self.ball = predicted_position
                self.goalkeeper = current_goalkeeper_pos
                self.reset_objects()
                return
    ...
    def move_goalkeeper(self):
        if self.mapped_input:
            ...
            if section_name in self.p.environment_dict:
                ...
                # Use the prediction method to adjust the new position
                predicted_position = self.predict_ball_position()
                predicted_x, predicted_y = predicted_position

                # Adjust the new position based on the prediction
                adjusted_position = (new_position[0] + predicted_x) / 2, (new_position[1] + predicted_y) / 2

                self.goalkeeper_start_pos = self.goalkeeper.copy()
                self.goalkeeper_target_pos = np.array(adjusted_position, dtype=np.float32)
                self.goalkeeper_progress = 0
                self.goalkeeper_animation = True

    def predict_ball_position(self):
        # Use a simple linear prediction based on current progress
        ball_speed = np.linalg.norm(self.ball_target_pos - self.ball_start_pos)
        if ball_speed > 0:
            direction = (self.ball_target_pos - self.ball_start_pos) / ball_speed
            # Use the current progress as the prediction time
            predicted_position = self.ball_start_pos + direction * ball_speed * self.ball_progress
            return predicted_position
        else:
            return self.ball

    def animate_goalkeeper_path(self):
        if self.goalkeeper_animation:
            self.goalkeeper_progress += 0.07  # Adjust speed as needed

            if self.goalkeeper_progress >= 1:
                self.goalkeeper_progress = 1
                self.goalkeeper_animation = False

            self.goalkeeper = self.goalkeeper_start_pos + (
                    self.goalkeeper_target_pos - self.goalkeeper_start_pos) * self.goalkeeper_progress
    ...
    def is_goal(self) -> bool:
        ...
        for progress in np.linspace(0, 1, num=100):
            current_ball_pos = self.ball_start_pos + (self.ball_target_pos - self.ball_start_pos) * progress
            current_goalkeeper_pos = self.goalkeeper_start_pos + (
                        self.goalkeeper_target_pos - self.goalkeeper_start_pos) * progress

            goalkeeper_box = pygame.Rect(float(current_goalkeeper_pos[0] - goalkeeper_width // 2),
                                         float(current_goalkeeper_pos[1] - goalkeeper_height // 2),
                                         goalkeeper_width, goalkeeper_height)

            ball_box = pygame.Rect(float(current_ball_pos[0] - ball_radius),
                                   float(current_ball_pos[1] - ball_radius),
                                   ball_radius * 2, ball_radius * 2)

            if pygame.Rect.colliderect(goalkeeper_box, ball_box):
                print("hellooo")
                return False
        print("yessss")
        return True

    def check_collision_in_frame(self, predicted_ball_pos, current_goalkeeper_pos):
        goalkeeper_width, goalkeeper_height = 60, 200
        goalkeeper_box = pygame.Rect(current_goalkeeper_pos[0] - goalkeeper_width // 2,
                                     current_goalkeeper_pos[1] - goalkeeper_height // 2,
                                     goalkeeper_width, goalkeeper_height)
        ball_box = pygame.Rect(predicted_ball_pos[0] - self.ball_radius,
                               predicted_ball_pos[1] - self.ball_radius,
                               self.ball_radius * 2, self.ball_radius * 2)

        return pygame.Rect.colliderect(goalkeeper_box, ball_box)
