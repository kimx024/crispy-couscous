...
class Environment(gym.Env):
    ...
    def reset(self, *, seed: int = None, options: dict = None) -> tuple[np.ndarray, dict]:
        ...
        self.state = np.array([self.ball, self.goalkeeper], dtype=np.float32)
        info_dict = {}
        return np.array(self.state), info_dict
    ...
    def reset_goalkeeper(self):
        self.goalkeeper = np.array([640, 280], dtype=np.float32)
        return self.goalkeeper

    def reset_ball(self):
        self.ball = np.array([600, 750], dtype=np.float32)
        return self.player
    ...
    def step(self, action):
        ...
        if self.ball_progress >= 1:
            ...
            if self.is_goal():
                ...
                print(
                    f"Terminated by goal. Current reward: {self.current_reward}."
                ...
                self.calculate_cumulative_reward()
                print(
                    f"Terminated by keeper. Current reward: {self.current_reward}."
                    f"Cumulative reward: {self.cumulative_reward}")
        ...
        self.state = np.concatenate([self.ball, self.goalkeeper], dtype=np.float32)
        ...
        #              }
        # print(info_step)
        return self.state, self.cumulative_reward, terminated, truncated_step, {}
    ...
    def animate_ball(self):
        ...
        if self.ball_animation:
            ...
            return self.ball, self.cumulative_reward
    ...
    def move_goalkeeper(self):
        if self.mapped_input:
            ...
            if section_name in self.p.environment_dict:
                ...
                self.goalkeeper_target_pos = np.array(new_position, dtype=np.float32)
                self.goalkeeper_progress = 0
    ...
    def animate_goalkeeper_path(self):
        if self.goalkeeper_animation:
            ...
            self.goalkeeper = (self.goalkeeper_start_pos + (self.goalkeeper_target_pos - self.goalkeeper_start_pos)
                               * self.goalkeeper_progress)
            return self.goalkeeper
...
# Run the environment normally
if __name__ == "__main__":
    output_path = "/Users/kim.lichtenberg/Desktop/rl-env/shifted-output.txt"
...
if __name__ == "__main__":
    ...
    obs = env.reset()
    for _ in range(1000):
        random_action, _states = model.predict(obs)
