    ...
    def animate_ball(self):
        ...
        if self.ball_animation:
            ...
            if self.ball_progress >= 1:
                ...
                print(f"I shoot to: {self.current_action}, {action_name} at coordinates {self.ball}")
...
#     output_file = "/Users/kim.lichtenberg/Desktop/rl-env/shifted-output.txt"
#
#     env = Environment(output_file)
...
#             state, reward, done, truncated, info = env.step(take_action)
#         counter += 1
#         # if counter >= 5000:
...
if __name__ == "__main__":
    ...
    # To keep track of training, I used ClearML
    task = Task.init(project_name="triple-rl", task_name="PPO Training")

    # Ensure the environment is wrapped correctly for vectorized environments
    env = make_vec_env(lambda: Environment(output_file, render_mode=None), n_envs=4)

    # Initialize the PPO model with MLP policy
    model = PPO("MlpPolicy", env, verbose=1, device="mps")

    # Optional: Get the parameters for the ClearML dashboard
    task.connect(model.get_parameters())

    # Train the model for a specified number of time steps
    model.learn(total_timesteps=10)

    # Save the trained model
    model.save("ppo_fifa_model")

    # Log the model to ClearML dashboard
    task.upload_artifact("ppo_fifa_model", "ppo_fifa_model.zip")
    ...
    # Close the ClearML dashboard
    task.close()

    # # Run the trained model in the environment and render
    # for _ in range(1000):
    #     take_action, _states = model.predict(observation)
    #     observation, rewards, done, info = env.step(take_action)
    #
