...
class Environment(gym.Env):
    ...
    STRIKE 1. It has to embody a virtual environment within the OpenAI Gymnasium resources
    ...
    3. The goalkeepers logic is hardcoded as follows:
        STRIKE A. Get input from the computer vision model about possible positions
        STRIKE B. Choose a random goal direction based on that section
        STRIKE C. Within that goal section, choose a random position to dive to
            ^- within more pondering, would this even matter?
            D. Dive to that position.
        STRIKE E. After the dive, return to a random x-axis position within the center
